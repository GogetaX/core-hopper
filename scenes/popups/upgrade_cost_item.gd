...
func InitCost(currency:String,target_currency:int):
	...
	$HList/cur_value.text = Global.CurrencyToString(target_currency)
	var my_currency = GlobalSave.GetCurrency(currency)
	$HList/max_value.text = Global.CurrencyToString(my_currency)
